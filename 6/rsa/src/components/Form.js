...
export class Form extends Component {
    ...
    render() {
      ...
      <main>
          ...
          <div className="text">
              <p>Взломанное d</p>
              <textarea rows={5} id="vzlomD"></textarea>
          </div>

          <div className="text">
              <p>Взломанный текст </p>
              <textarea rows={5} id="vzlomText"></textarea>
          </div>
      </main>
      )
    }
    ...
    vzlom(){
        ...
        let text = document.getElementById('vzlomText')
        let d = document.getElementById('vzlomD')

        console.log(e, N, cryptogramm)
        if(!e || !N || !cryptogramm){
            alert('Заполните все поля!')
        }
        else{
            let res = apiPos({'e': e, 'N': N, 'text': cryptogramm}, '/vzlom')
            res.then(res => {
                if(res.errorN === 'yes') alert(res.p)
                if(res.error === 'error') alert('Ошибка расшифрования!')
                if(res.status === 'ok'){
                    text.innerHTML = res.result.mes
